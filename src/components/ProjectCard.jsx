  ...
  return (
    ...
    <h5 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white overflow-hidden h-10">
      {project.name.length>23 ? project.name.toUpperCase().slice(0,20)+"..." : project.name.toUpperCase()}
        ...
        {theme === "dark" ? (
      <div className="bg-purple-200 max-w-10 p-1 rounded dark:bg-logo-purple dark:text-white" style={{display:"flex", gap:"2px", alignItems:"center"}}>
          <img src={imgWhiteLikes} alt="likes" style={{width:"20px"}} /> {project.likes.length}</div>
          ) : (
      <div className="bg-purple-200 max-w-10 p-1 rounded dark:bg-logo-purple dark:text-white" style={{display:"flex", gap:"2px", alignItems:"center"}}>
          <img src={imgLikes} alt="likes" style={{width:"20px"}} /> {project.likes.length}</div>
          ) }
    <Card className="p-0 min-h-[50px] dark:border-1 dark:border-purple-200">
      <div className="flex flex-row justify-center gap-5 items-center">
        ...
        <h5 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white overflow-hidden h-10 flex items-center">{project.user.username}</h5>
      </div>
    </Card>

      {project.collaborators.length > 0 &&
    <Card className="dark:border-1 dark:border-purple-200">
      <div className="flex flex-row justify-center gap-2 items-center"><span className="text-md font-bold tracking-tight text-gray-900 dark:text-white overflow-hidden h-10 flex items-center">Collaborators:</span> {
        <Avatar.Group>
                ...
                {project.collaborators.length > 4 && <Avatar.Counter total={project.collaborators.length-4} /> }
